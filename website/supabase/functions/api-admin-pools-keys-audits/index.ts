...
serve(async (req) => {
  ...
  try {
    ...
    // Get audit logs for both secrets
    ...
    console.log(`Retrieved ${audits?.length || 0} audit logs for pool config: ${id}`);

    return new Response(
      JSON.stringify({ audits: audits || [] }),
      {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      }
    );
  } catch (error) {
    console.error('Error in api-admin-pools-keys-audits:', error);
